...
import "./v2-pool-stable/contracts/StablePool.sol";
...
contract StablePhantomPool is StablePool {
    ...
    event CachedProtocolSwapFeePercentageUpdated(
        ...
        )
    {
        ...
        for (uint256 i = 0; i < params.tokens.length; i++) {
            if (params.rateProviders[i] != IRateProvider(0)) {
                _updateTokenRateCache(
                    params.tokens[i],
                    params.rateProviders[i],
                    params.tokenRateCacheDurations[i]
                );
        ...
        uint256 bptIndex;
        for (
            bptIndex = params.tokens.length;
            bptIndex > 0 && params.tokens[bptIndex - 1] > IERC20(this);
            bptIndex--
        ) {
        ...
        IRateProvider[] memory tokensAndBPTRateProviders = new IRateProvider[](
        ...
        _rateProvider0 = (tokensAndBPTRateProviders.length > 0)
            ? tokensAndBPTRateProviders[0]
            : IRateProvider(0);
        _rateProvider1 = (tokensAndBPTRateProviders.length > 1)
            ? tokensAndBPTRateProviders[1]
            : IRateProvider(0);
        _rateProvider2 = (tokensAndBPTRateProviders.length > 2)
            ? tokensAndBPTRateProviders[2]
            : IRateProvider(0);
        _rateProvider3 = (tokensAndBPTRateProviders.length > 3)
            ? tokensAndBPTRateProviders[3]
            : IRateProvider(0);
        _rateProvider4 = (tokensAndBPTRateProviders.length > 4)
            ? tokensAndBPTRateProviders[4]
            : IRateProvider(0);
    ...
    function _onSwapGivenIn(
        ...
        (uint256 virtualSupply, uint256[] memory balances) = _dropBptItem(
            balancesIncludingBpt
        );

        if (request.tokenIn == IERC20(this)) {
            amountOut = _onSwapTokenGivenBptIn(
                request.amount,
            ...
            amountOut = _onSwapBptGivenTokenIn(
                request.amount,
            ...
            if (protocolSwapFeePercentage > 0) {
                ...
                balances[newIndexIn] = balances[newIndexIn].add(
                    amountInWithFee
                );
    ...
    function _onSwapGivenOut(
        ...
        if (request.tokenIn == IERC20(this)) {
            amountIn = _onSwapBptGivenTokenOut(
                request.amount,
                _skipBptIndex(indexOut),
                virtualSupply,
                balances
            ...
            amountIn = _onSwapTokenGivenBptOut(
                request.amount,
                _skipBptIndex(indexIn),
                virtualSupply,
                balances
            ...
            if (protocolSwapFeePercentage > 0) {
                ...
                balances[newIndexOut] = balances[newIndexOut].sub(
                    request.amount
                );
    ...
    function _onSwapBptGivenTokenOut(
        ...
        bptIn = StableMath._calcBptInGivenExactTokensOut(
            amp,
            balances,
            amountsOut,
            virtualSupply,
    ...
    function _onSwapBptGivenTokenIn(
        ...
        (uint256 amp, ) = _getAmplificationParameter();
        bptOut = StableMath._calcBptOutGivenExactTokensIn(
            amp,
            balances,
            amountsIn,
            virtualSupply,
    ...
    function _trackDueProtocolFeeByInvariantIncrement(
        ...
        uint256 postSwapInvariant = StableMath._calculateInvariant(
            amp,
            postSwapBalances,
            false
        );
        ...
        if (invariantRatio > FixedPoint.ONE) {
            ...
            _dueProtocolFeeBptAmount = _dueProtocolFeeBptAmount.add(
                protocolFeeAmount
            );
     ...
     */
    function _trackDueProtocolFeeByBpt(
        uint256 bptAmount,
        uint256 protocolSwapFeePercentage
    ) private {
        uint256 feeAmount = _addSwapFeeAmount(bptAmount).sub(bptAmount);

        uint256 protocolFeeAmount = feeAmount.mulDown(
            protocolSwapFeePercentage
        );
        _dueProtocolFeeBptAmount = _dueProtocolFeeBptAmount.add(
            protocolFeeAmount
    ...
    function _onJoinPool(
        ...
        uint256[] memory,
        bytes memory userData
    ) internal override returns (uint256, uint256[] memory, uint256[] memory) {
    ...
    function _getRateProvider(
        IERC20 token
    ) internal view returns (IRateProvider) {
        // prettier-ignore
    ...
    function getTokenRate(IERC20 token) public view virtual returns (uint256) {
        ...
        return
            tokenRateCache == bytes32(0)
                ? FixedPoint.ONE
                : tokenRateCache.getRate();
    }
    ...
    function getTokenRateCache(
        ...
        _require(
            _getRateProvider(token) != IRateProvider(0),
    ...
    function _cacheTokenRateIfNecessary(IERC20 token) internal {
        ...
        if (cache != bytes32(0)) {
            (uint256 duration, uint256 expires) = _tokenRateCaches[token]
                .getTimestamps();
    ...
    function _isOwnerOnlyAction(
        ...
        return
            (actionId ==
                getActionId(this.setTokenRateCacheDuration.selector)) ||
    ...
    function _addBptItem(
        uint256[] memory amounts,
        uint256 bptAmount
    ) internal view returns (uint256[] memory amountsWithBpt) {
        amountsWithBpt = new uint256[](amounts.length + 1);
        for (uint256 i = 0; i < amountsWithBpt.length; i++) {
            amountsWithBpt[i] = i == _bptIndex
                ? bptAmount
                : amounts[i < _bptIndex ? i : i - 1];
        }
    }
    ...
    function getRate() public view override returns (uint256) {
        (, uint256[] memory balancesIncludingBpt, ) = getVault().getPoolTokens(
            getPoolId()
        );
